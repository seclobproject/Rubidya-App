...
class Followerinnerlist extends StatefulWidget {
  Followerinnerlist({Key? key,required this.id}) : super(key: key);

   String? id;

  @override
  State<Followerinnerlist> createState() => _FollowingInnerListState();
}

class _FollowingInnerListState extends State<Followerinnerlist> {
  ...
  Future<void> _followingFollowList() async {
    ...
    var response = await HomeService.getfollowerinner(widget.id);

    if (response != null && response['followers'] != null) {
      List<dynamic> followingData = response['followers'];
